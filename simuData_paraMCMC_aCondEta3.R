  ...
  if (nP>1){
    parallelOut <- foreach(p = 1:nP) %dopar%{
      ...
      if (length(zVec.p)==0){
        ...
        zVec.p = out$zVec
        K.p = out$K 
        LambdaMat.p = out$LambdaMat 
        clusterLabels.p = sort(unique(zVec.p))
        
        out.p = list(procId=p, clusterLabels.p=clusterLabels.p, CtIds.p=CtIds.p, zVec.p = zVec.p, K.p=K.p, LambdaMat.p = LambdaMat.p)
      }
      out.p
    }	
